...
export function FormMeta({ onSubmit }: FormMetaProps) {
  ...
  const metaDiaria =
    form.valorMeta > 0 && form.diasTrabalho > 0
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (form.valorMeta <= 0) {
      setErro("Informe a meta de lucro.");
      return;
    }
    if (form.diasTrabalho <= 0 || form.diasTrabalho > 31) {
      setErro("Dias de trabalho deve ser entre 1 e 31.");
      return;
    }
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {TIPOS.map((t) => (
            ...
            >
              <i
                className={`ti ti-${ICONES_TIPO[t]} text-[20px]`}
                aria-hidden
              />
      ...
      {metaDiaria > 0 && (
        <div className="rounded-xl bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-100 dark:border-emerald-900 p-4">
          ...
          <p className="text-xs text-emerald-600/70 dark:text-emerald-500 mt-1">
            {formatBRL(form.valorMeta)} de lucro em {form.diasTrabalho} dias de
            trabalho
          </p>
        </div>
      )}
